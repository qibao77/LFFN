...
def show_heatmap(input_array):
	array_min = np.min(input_array)
	array_max = np.max(input_array)
	input_map = (input_array-array_min)/(array_max-array_min)
	ax = sns.heatmap(input_map,
					 cmap=plt.cm.rainbow,
					 annot=False)
	# ax.set_title('')
...
def build_input_image(image, width=0, height=0, scale=1, alignment=0):
	"""
	build input image from file.
	crop, adjust the image alignment for the scale factor, resize
	"""
